...
source local.env
PACKAGE_NAME=tjlamp-bot

function usage() {
  echo "Usage: $0 [--install,--uninstall,--update,--env,--urls]"
}
...
function install() {
  ...
  bx wsk package create $PACKAGE_NAME\
    ...
    -p cloudantIAMAPIKey $CLOUDANT_iamapikey\
  ...
  echo "Adding app interactivity processing"
  bx wsk action create $PACKAGE_NAME/slackapp-interactive actions/slackapp-interactive.js\
    --web true --annotation final true
...
function uninstall() {
  ...
  bx wsk action delete $PACKAGE_NAME/slackapp-interactive
...
function showurls() {
  ...
  echo Interactive Subscription Request URL:
  echo https://$OPENWHISK_API_HOST/api/v1/web$(bx wsk list | grep 'slackapp/slackapp-interactive' | awk '{print $1}')
}

function update() {
  bx wsk action update $PACKAGE_NAME/slackapp-register    actions/slackapp-register.js
  bx wsk action update $PACKAGE_NAME/slackapp-interactive actions/slackapp-interactive.js
